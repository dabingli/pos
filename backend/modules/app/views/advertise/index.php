...
<script type="text/javascript">
    ...
    var TableInit = function () {
        ...
        oTableInit.Init = function () {
            $('#table').bootstrapTable({
                url: '<?php echo Url::toRoute(['advertise/list']) ?>',         //请求后台的URL（*）
                method: 'post',                      //请求方式（*）
                ...
                columns: [{
                ...
                },
                    {
                        ...
                        formatter:function(value,row,index){
                            ...
                            if(row.image!=null){
                                var url = row.image;
                                s = '<a class = "view img"  href="javascript:void(0)"><img style="width:30px;height:40px;"  src="'+url+'" /></a>';
                            }
                            return s;
                    ...
                    {
                        formatter: function (value, row, index) {
                            var open = '<?php echo AppAdvertise::START; ?>';
                            var status = row.o_status == open ? 'checked' : '';
                            var editStatus = row.o_status == open ? '<?php echo AppAdvertise::STOP; ?>' : open;
    ...
    $('#btn_add').click(function(){
        $.ajax({
            ...
            url:"<?php echo Url::toRoute(['advertise/add']) ?>",
    ...
    function edit($id){
        ...
        $.ajax({
            type:"POST",
            async:true,//false时为同步true为异步一般是异步
            url:"<?php echo Url::toRoute(['advertise/edit']) ?>",
