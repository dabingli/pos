...
<script type="text/javascript">
    ...
    function checkForm(){
        ...
        var min_cashback = $('input[name="config[min_cashback]"]').val();
        // if(!checkCash(min_cashback)){
        //     showError('最小提现金额(返现)必须大于0且是100的整数倍');
        //     res = false;
        // }

        var min_cash_amount = $('input[name="config[min_cash_amount]').val();
        // if(!checkCash(min_cash_amount)){
        //     showError('最小提现金额(分润)必须大于0且是100的整数倍');
        //     res = false;
        // }
    ...
    function checkCash(num){
        ...
        if((parseInt(num) % 100) != 0) return false;
