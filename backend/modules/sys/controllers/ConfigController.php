...
class ConfigController extends SController
{
    ...
    public function actionAjaxEdit()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            return $model->save() ? $this->message('操作成功', $this->redirect(Yii::$app->request->referrer)) : $this->message($this->analyErr($model->getFirstErrors()), $this->redirect(Yii::$app->request->referrer), 'error');
        }
    ...
    public function actionUpdateInfo()
    {
        ...
        if ($request->isAjax) {
            ...
            foreach ($config as $key => $value) {
                if ($model = Config::find()->where([
                    'name' => $key
                ])->one()) {
                    $model->value = is_array($value) ? serialize($value) : $value;
                    if (! $model->save()) {
                        return $this->message($this->analyErr($model->getFirstErrors()), $this->redirect(['edit-all']));
                    }
                } else {
                    return $this->message("配置不存在,请刷新页面", $this->redirect(['edit-all']));
                }
            }
            
            Yii::$app->debris->clearConfigCache();
            return $this->message("修改成功", $this->redirect(['edit-all']));
        }
