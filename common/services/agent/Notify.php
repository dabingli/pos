...
use common\models\agent\Notify as NotifyModel;
use common\models\agent\NotifyManager;
...
class Notify extends Service
{
    ...
    public function pullAnnounce($manager_id)
    {
        ...
        $model = NotifyManager::find()->where([
            'manager_id' => $manager_id,
            'type' => NotifyModel::TYPE_ANNOUNCE
        ])
            ->orderBy('id desc')
            ->asArray()
            ->one();
        ...
        $notifys = NotifyModel::find()->where([
            'type' => NotifyModel::TYPE_ANNOUNCE,
        ...
        foreach ($notifys as $notify) {
            $rows[] = [
                $notify['id'],
                $manager_id,
                NotifyModel::TYPE_ANNOUNCE,
                $notify['created_at'],
                time()
            ];
        }
        
        ! empty($rows) && Yii::$app->db->createCommand()
            ->batchInsert(NotifyManager::tableName(), $fields, $rows)
            ->execute();
    }
