...
use common\models\agent\AgentMenuCate;

/**
 * This is the model class for table "{{%agent_menu}}".
 *
 * @property int $id
 ...
 */
class AgentMenu extends \common\models\common\BaseModel
{
    ...
    {
        $data = AgentMenu::find()->andFilterWhere([
            'status' => $status
        ]);
        // 关闭开发模式
        if (empty(Yii::$app->debris->config('sys_dev'))) {
            $data = $data->andWhere([
                'dev' => StatusEnum::DISABLED
            ]);
        }
        ...
        // 获取当前所有的权限信息
        $auth = Yii::$app->services->agent->isAuperAdmin() ? false : AuthHelper::getAuth();
    ...
    {
        return $this->hasOne(AgentMenuCate::class, [
            'id' => 'cate_id'
        ]);
    }
