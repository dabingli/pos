...
class UploadHelper
{
        ...
        switch ($this->drive) {
            ...
            case Attachment::DRIVE_OSS:
                $adapter = new OssAdapter([
                    'accessId' => $sysConfig['storage_aliyun_accesskeyid'],
                    'accessSecret' => $sysConfig['storage_aliyun_accesskeysecret'],
    ...
    protected function watermark()
    {
        ...
        if ($coordinate = DebrisHelper::getWatermarkLocation($absolutePath, $watermarkImg, $local)) {
            ...
            Image::watermark($absolutePath, $watermarkImg, $coordinate)->save($absolutePath, [
                'quality' => 100
            ]);
        }
    ...
    protected function compress()
    {
        ...
        foreach ($compressibility as $key => $item) {
            if ($this->baseInfo['size'] >= $tmpMinSize && $this->baseInfo['size'] < $key && $item < 100) {
                ...
                Image::thumbnail($absolutePath, $imgInfo[0], $imgInfo[1])->save($absolutePath, [
                    'quality' => $item
                ]);
    ...
    public function getBaseInfo()
    {
        ...
        $this->baseInfo['url'] = CdnHelper::Url($path);
        // 写入数据库
        ...
        $this->baseInfo['path'] = $attachment->path;
        if (! $attachment->save()) {
